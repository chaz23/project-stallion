...
while i < numRaces:
	...
	betfairFileName = dataPath + '/Betfair-' + str(raceName[0]) + '.csv\n'
	williamHillFileName = dataPath + '/William Hill-' + str(raceName[0]) + '.csv\n'
	centrebetFileName = dataPath + '/Centrebet-' + str(raceName[0]) + '.csv\n'
	...
	while currentTime < (raceStartTime - datetime.timedelta(minutes=120)):
		time.sleep(60)
		
	# launch subprocess
	print('launching subprocess ' + str(i + 1))
	os.chdir(targetPath)
	subprocess.Popen("python v1.6.0.py", shell=True)
