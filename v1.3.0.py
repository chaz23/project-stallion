...
centrebetFile = open('C:/Users/Charith/Desktop/testcentrebet.txt', 'w')
unibetFile = open('C:/Users/Charith/Desktop/unibet.txt', 'w')

# URL to Betfair race
urlBetfair = 'https://www.betfair.com.au/exchange/plus/horse-racing/market/1.134085875'

# URL to William Hill race
urlWilliamHill = 'https://www.williamhill.com.au/horse-racing/67336112/england-perth-1'

# URL to centrebet race
urlCentrebet = 'http://centrebet.com/#Racing/67336112'

# URL to unibet race
urlUnibet = 'https://www.unibet.com.au/racing#/event/1592924/win?_k=0d3z9i'
...
placeName1_unibet = '//*[@id="center-column"]/div/div/div/div[3]/table/tbody/tr['
placeName2_unibet = ']/td['
placeName3_unibet = ']/div/button'
...
startTime = datetime.datetime.strptime('Sep 11 2017  19:41', '%b %d %Y %H:%M')
...
while currentTime < (startTime - datetime.timedelta(minutes=1)):
	...
	if handle == 0: # looking at the Betfair page
		...
		# check whether odds are [fixed], [fixed, best tote], [fixed, mid tote, tote + 10%]
		# find first horse that isnt scratched
		i = 0
		j = 1
		while i == 0:
			placeName_unibet = placeName1_unibet + str(j) + placeName2_unibet + '6' + placeName3_unibet
			try:
				test = browser.find_element_by_xpath(placeName_unibet)
				i = 1
			except:
				j = j + 1

		# find which odd types are on the page
		try:
			placeName_unibet = placeName1_unibet + str(j) + placeName2_unibet + '11' + placeName3_unibet
			test = browser.find_element_by_xpath(placeName_unibet)
			j = 11
		except:
			try:
				placeName_unibet = placeName1_unibet + str(j) + placeName2_unibet + '10' + placeName3_unibet
				test = browser.find_element_by_xpath(placeName_unibet)
				j = 10
			except:
				try:
					placeName_unibet = placeName1_unibet + str(j) + placeName2_unibet + '9' + placeName3_unibet
					test = browser.find_element_by_xpath(placeName_unibet)
					j = 9
				except:
					try:
						placeName_unibet = placeName1_unibet + str(j) + placeName2_unibet + '8' + placeName3_unibet
						test = browser.find_element_by_xpath(placeName_unibet)
						j = 8
					except:
						placeName_unibet = placeName1_unibet + str(j) + placeName2_unibet + '7' + placeName3_unibet
						test = browser.find_element_by_xpath(placeName_unibet)
						j = 7
		print(j)
		if j == 7:
			# find horse names by class
			nameElement_unibet = browser.find_elements_by_class_name('event-runner__name')
			length = len(nameElement_unibet)
			
			i = 1
			k = 0
			while i <= length:
				placeName_unibet = placeName1_unibet + str(i) + placeName2_unibet + '7' + placeName3_unibet
				try:
					placeElement_unibet = browser.find_element_by_xpath(placeName_unibet)
					unibetFile.write(nameElement_unibet[k].text + ',')
			...
			k = 0
			while i <= length:
				placeName_unibet = placeName1_unibet + str(i) + placeName2_unibet + '8' + placeName3_unibet
				try:
					placeElement_unibet = browser.find_element_by_xpath(placeName_unibet)
					unibetFile.write(nameElement_unibet[k].text + ',')
					unibetFile.write(placeElement_unibet.text + ',')
			...
			k = 0
			while i <= length:
				placeName_unibet = placeName1_unibet + str(i) + placeName2_unibet + '9' + placeName3_unibet
				try:
					placeElement_unibet = browser.find_element_by_xpath(placeName_unibet)
					unibetFile.write(nameElement_unibet[k].text + ',')
					unibetFile.write(placeElement_unibet.text + ',')
